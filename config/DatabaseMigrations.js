...
class DatabaseMigrations {
  ...
  async createNewStorageTables() {
    try {
      ...
      if (!diskIOTableExists) {
        ...
        // Create indexes
        await db.query(
          `CREATE INDEX idx_disk_io_host_device_timestamp ON disk_io_stats(host, device_name, scan_timestamp)`
        );
        await db.query(
          `CREATE INDEX idx_disk_io_host_pool_timestamp ON disk_io_stats(host, pool, scan_timestamp)`
        );
      ...
      if (!poolIOTableExists) {
        ...
        // Create indexes
        await db.query(
          `CREATE INDEX idx_pool_io_host_pool_timestamp ON pool_io_stats(host, pool, scan_timestamp)`
        );
        await db.query(
          `CREATE INDEX idx_pool_io_host_timestamp ON pool_io_stats(host, scan_timestamp)`
        );
        await db.query(`CREATE INDEX idx_pool_io_timestamp ON pool_io_stats(scan_timestamp)`);
        await db.query(
          `CREATE INDEX idx_pool_io_pool_timestamp ON pool_io_stats(pool, scan_timestamp)`
        );
      }
  ...
  async migratePCIDevicesTable() {
    ...
    const columnsToAdd = [{ name: 'ppt_capable', definition: 'BOOLEAN DEFAULT 0' }];
  ...
  async migrateTasksTable() {
    ...
    const columnsToAdd = [{ name: 'metadata', definition: 'TEXT' }];
  ...
  async cleanupSwapAreasTable() {
    try {
      ...
      await db.query(
        `CREATE INDEX IF NOT EXISTS idx_swap_areas_host_timestamp ON swap_areas(host, scan_timestamp)`
      );
      await db.query(
        `CREATE INDEX IF NOT EXISTS idx_swap_areas_timestamp ON swap_areas(scan_timestamp)`
      );
      await db.query(
        `CREATE INDEX IF NOT EXISTS idx_swap_areas_utilization ON swap_areas(utilization_pct)`
      );
