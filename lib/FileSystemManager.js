...
export const writeFileContent = async (filePath, content, options = {}) => {
    ...
    try {
        ...
        if (options.uid !== undefined || options.gid !== undefined) {
            await fs.chown(normalizedPath, options.uid || -1, options.gid || -1);
        }
        
        // Set permissions if specified
        if (options.mode !== undefined) {
            await fs.chmod(normalizedPath, options.mode);
        }
...
export const copyItem = async (sourcePath, destPath, options = {}) => {
    ...
    try {
        ...
        if (stats.isDirectory()) {
            // Use cp command for recursive directory copy
            const result = await executeCommand(
                `pfexec cp -r "${sourceValidation.normalizedPath}" "${destValidation.normalizedPath}"`
            );
