 ...
 */
const getItemInfo = async targetPath => {
 ...
 */
const writeFileContent = async (filePath, content, options = {}) => {
  const validation = validatePath(filePath);
  if (!validation.valid) {
    throw new Error(validation.error);
  }
 ...
 * @returns {Promise<void>}
 */
const createDirectory = async (dirPath, options = {}) => {
  const validation = validatePath(dirPath);
  if (!validation.valid) {
    throw new Error(validation.error);
  }
 ...
 * @returns {Promise<void>}
 */
const deleteItem = async (targetPath, options = {}) => {
  const validation = validatePath(targetPath);
  if (!validation.valid) {
    throw new Error(validation.error);
  }
 ...
 * @returns {Promise<void>}
 */
const moveItem = async (sourcePath, destPath) => {
 ...
 * @returns {Promise<void>}
 */
const copyItem = async (sourcePath, destPath, options = {}) => {
  const sourceValidation = validatePath(sourcePath);
  if (!sourceValidation.valid) {
    throw new Error(`Source path error: ${sourceValidation.error}`);
  }
 ...
 * @returns {Promise<void>}
 */
const createArchive = async (sourcePaths, archivePath, format) => {
  const fileBrowserConfig = config.getFileBrowser();
