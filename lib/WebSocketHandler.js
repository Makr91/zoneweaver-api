 ...
 */
const setupVncWebSocket = (ws, zoneName, sessionInfo, connectionTracker, performSmartCleanup) => {
 ...
 */
const extractZoneFromHeaders = async request => {
  ...
  try {
    ...
    if (fsModule.existsSync(sessionManager.pidDir)) {
      const pidFiles = fsModule
...
const handleVncWebSocketUpgrade = async (url, request, socket, head, wss) => {
  ...
  if (zonePathMatch) {
    ...
    if (zonePathMatch) {
      ...
      zoneName = await extractZoneFromHeaders(request);
      if (!zoneName) {
        socket.destroy();
        return;
      }
