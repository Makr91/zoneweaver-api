...
class ZloginAutomation {
  ...
  async _stepCommand(step, vars, globalDeadline) {
    ...
    // Wait for the exit code marker (including the colon to ensure full line is captured)
    const result = await ptyManager.waitForPattern(this.zoneName, `${marker}:`, timeout, globalDeadline);
    if (!result.matched) {
    ...
    const markerMatch = strippedBuffer.match(new RegExp(`${marker}:(\\d+)`));
