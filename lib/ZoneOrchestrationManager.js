...
import { calculateShutdownOrder, calculateStartupOrder, extractZonePriority } from './ZoneOrchestrationUtils.js';
...
export const enableZoneOrchestration = async () => {
  try {
    ...
    if (runningZones.length > 0) {
      ...
      // Execute startup orchestration for previously running zones
      const startupResult = await executeZoneStartupOrchestration(runningZones);
      
      return {
        success: true,
        message: `Zone orchestration enabled successfully - ${runningZones.length} zones restarted`,
        zones_restarted: startupResult.zones_started || [],
        zones_failed: startupResult.zones_failed || [],
      };
    }
 ...
 */
export const executeZoneStartupOrchestration = async (specificZones = null, strategy = 'parallel_by_priority', options = {}) => {
  ...
  try {
    ...
    let zonesResult;
    if (specificZones) {
      // Start specific zones that were provided
      const zones = [];
      for (const zoneName of specificZones) {
...
export const getAutobootZones = async () => {
  try {
    ...
    for (const dbZone of dbZones) {
      // Check if zone has autoboot enabled
      const isAutoboot = dbZone.configuration?.autoboot === 'true';
