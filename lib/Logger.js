...
const createCategoryLogger = (category, filename) => {
  ...
  categoryTransport.on('archive', zipFilename => {
    try {
      ...
      // Use process.stderr since logging system might not be ready
      process.stderr.write(
        `Warning: Failed to move ${category} log archive ${zipFilename}: ${error.message}\n`
      );
    }
  });
...
export const artifactLogger = createCategoryLogger('artifact', 'artifact');
...
const appTransport = new DailyRotateFile({
  ...
  createSymlink: true,
  symlinkName: path.join('..', 'application.log'), // Relative path to parent directory
});
...
const errorTransport = new DailyRotateFile({
  ...
  createSymlink: true,
  symlinkName: path.join('..', 'error.log'), // Relative path to parent directory
});
...
appTransport.on('archive', zipFilename => {
  try {
    ...
    // Use process.stderr since logging system might not be ready
    process.stderr.write(`Warning: Failed to move log archive ${zipFilename}: ${error.message}\n`);
  }
});
...
errorTransport.on('archive', zipFilename => {
  try {
    ...
    // Use process.stderr since logging system might not be ready
    process.stderr.write(
      `Warning: Failed to move error log archive ${zipFilename}: ${error.message}\n`
    );
  }
});
...
const safeLog = (logger, level, message, meta = {}) => {
  try {
    ...
    // Fallback to stderr if winston fails - avoid console statements
