...
const createCategoryLogger = (category, filename) => {
  ...
  const categoryTransport = new DailyRotateFile({
    ...
    format: logFormat,
    createSymlink: true,
    symlinkName: path.join(logDir, `${filename}.log`),
  });
  ...
  // All errors also go to shared error log (only create once, not per category)
  if (filename !== 'error') {
    ...
    // Move error archives to archives/ folder too
