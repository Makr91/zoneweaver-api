...
export const getAggregates = async (req, res) => {
    try {
        ...
        if (live === 'true' || live === true) {
            ...
            let command = 'pfexec dladm show-aggr -p -o link,policy,addrpolicy,lacpactivity,lacptimer,flags';
            if (extended === 'true' || extended === true) {
                command = 'pfexec dladm show-aggr -x -p -o link,port,speed,duplex,state,address,portstate';
            }
            ...
            if (!result.success) {
                return res.status(500).json({
                    error: 'Failed to get live aggregate data',
            ...
            if (extended === 'true' || extended === true) {
                // Parse extended output with port details
                const lines = result.output.split('\n').filter(line => line.trim());
                const aggrMap = new Map();
                
                lines.forEach(line => {
                    const [link, port, speed, duplex, portState, address, portStateInfo] = line.split(':');
                ...
                aggregates = Array.from(aggrMap.values());
            } else {
                // Parse basic output
                aggregates = result.output.split('\n')
                    .filter(line => line.trim())
                    .map(line => {
                        const [link, aggrPolicy, addrpolicy, lacpactivity, lacptimer, flags] = line.split(':');
                        
                        return {
                            link,
                            class: 'aggr',
                            policy: aggrPolicy,
                            address_policy: addrpolicy,
                    ...
                    .filter(aggr => {
                        if (policy && aggr.policy !== policy) return false;
                        // Note: live query doesn't include state directly
                        return true;
                    })
        ...
        // Get data from database (monitoring data) - only get the latest record per aggregate
...
export const getAggregateDetails = async (req, res) => {
    try {
        ...
        const { live = false, extended = false, lacp = false } = req.query;

        if (live === 'true' || live === true) {
            console.log(`🔍 Getting live aggregate details for ${aggregate}...`);
            
            // Get basic aggregate info - policy, address policy, LACP settings, flags
            console.log('📋 Step 1: Getting basic aggregate configuration...');
            const basicResult = await executeCommand(`pfexec dladm show-aggr ${aggregate} -p -o link,policy,addrpolicy,lacpactivity,lacptimer,flags`);
            
            if (!basicResult.success) {
                console.log('❌ Aggregate not found or command failed:', basicResult.error);
            ...
            const [link, policy, addrpolicy, lacpactivity, lacptimer, flags] = basicResult.output.split(':');
            console.log('✅ Basic aggregate info retrieved:', { link, policy, addrpolicy, lacpactivity, lacptimer, flags });
            ...
            // Always get extended port information (speed, duplex, state, addresses, port states)
            console.log('📋 Step 2: Getting extended port information...');
            const extendedResult = await executeCommand(`pfexec dladm show-aggr ${aggregate} -x -p -o link,port,speed,duplex,state,address,portstate`);
            
            if (extendedResult.success) {
                console.log('✅ Extended info retrieved:', extendedResult.output);
                const ports = extendedResult.output.split('\n')
                    .filter(line => line.trim())
                    .map(line => {
                        const [linkName, port, speed, duplex, portState, address, portStateInfo] = line.split(':');
                        
                        // Skip the aggregate summary line (has empty port)
                        if (!port) {
                            return {
                                link: linkName,
                                is_aggregate: true,
                                speed: speed ? speed.replace('Mb', '') : null,
                                duplex,
                                state: portState,
                                address: address ? address.replace(/\\/g, '') : null, // Remove escape characters
                                port_state: portStateInfo
                            };
                        }
                ...
                aggregateDetails.ports = ports;
                console.log('✅ Processed port information:', ports);
            } else {
                console.warn('⚠️  Failed to get extended port info:', extendedResult.error);
            }

            // Always get LACP information (aggregatable, sync, collecting, distributing states)
            console.log('📋 Step 3: Getting LACP information...');
            const lacpResult = await executeCommand(`pfexec dladm show-aggr ${aggregate} -L -p -o link,port,aggregatable,sync,coll,dist,defaulted,expired`);
            
            if (lacpResult.success) {
                console.log('✅ LACP info retrieved:', lacpResult.output);
                const lacpInfo = lacpResult.output.split('\n')
                    .filter(line => line.trim())
                    .map(line => {
                        const [linkName, port, aggregatable, sync, coll, dist, defaulted, expired] = line.split(':');
                        return {
                            link: linkName || aggregate, // Use aggregate name if link is empty
                            port,
                ...
                aggregateDetails.lacp_info = lacpInfo;
                console.log('✅ Processed LACP information:', lacpInfo);
            } else {
                console.warn('⚠️  Failed to get LACP info:', lacpResult.error);
            }

            // Extract port list for easier frontend access
            if (aggregateDetails.ports) {
                const portNames = aggregateDetails.ports
                    .filter(p => !p.is_aggregate && p.port)
                    .map(p => p.port);
                aggregateDetails.port_list = portNames;
                console.log('✅ Extracted port list:', portNames);
            }

            console.log('✅ Complete aggregate details assembled');
            return res.json(aggregateDetails);
        }
