...
class StorageCollector {
  ...
  async collectDatasetData() {
    try {
      ...
      for (const dataset of zoneDatasets) {
        ...
        try {
          const { stdout: propsOutput } = await execProm(`zfs get all "${dataset.name}"`, {
            timeout,
          });
  ...
  async collectDiskData() {
    try {
      ...
      const { stdout: formatOutput } = await execProm('echo | pfexec format | grep "^[ ]*[0-9]"', {
        timeout,
      });
  ...
  async collectComprehensiveIOStats() {
    try {
      ...
      const { stdout: iostatOutput } = await execProm('pfexec zpool iostat -l -H -v 1 2', {
        timeout,
      });
