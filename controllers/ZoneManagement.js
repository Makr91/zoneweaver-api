...
/**
 * Get next available provisioning IP from the configured range
 * @param {Array<string>} excludeIps - IPs to exclude from selection
 * @returns {Promise<string|null>} Next available IP or null
 */
const getNextProvisioningIP = async (excludeIps = []) => {
  ...
  // Get all used IPs from zones in DB
  ...
  for (let i = start; i <= end; i++) {
    const ip = longToIp(i);
    if (!usedIps.has(ip) && !excludeIps.includes(ip)) {
      return ip;
    }
  }
...
const createCloneSnapshots = async sourceZone => {
  ...
  if (zoneConfig.disks && zoneConfig.disks.boot && zoneConfig.disks.boot.dataset) {
    ...
    // zadm format
    bootDataset = zoneConfig.bootdisk.path; // e.g. rpool/zones/zone1/boot
  }
    ...
    await Promise.all(
      zoneConfig.disks.additional.map(async disk => {
        ...
        if (snapResult.success) {
          additionalSnapshots.push({ dataset, snapshotName });
        } else {
          log.api.warn(`Failed to snapshot additional disk ${dataset}`, {
...
const buildCloneMetadata = async (sourceZone, requestBody, snapshotInfo, newZoneName) => {
  ...
  // Process additional disks
  if (sourceConfig.disks && sourceConfig.disks.additional) {
    for (const disk of sourceConfig.disks.additional) {
      const dataset = `${disk.pool}/${disk.dataset}/${disk.volume_name}`;
      const snap = snapshotInfo.additionalSnapshots.find(s => s.dataset === dataset);

      if (snap) {
        // Clone from snapshot
        disks.additional.push({
          ...disk,
          source: {
            type: 'template',
        ...
        // Create empty (snapshot failed or not found)
        disks.additional.push({ ...disk });
      }
    }
  }
  ...
  const networks = [];
  const assignedIps = [];
  if (sourceConfig.networks) {
    for (const net of sourceConfig.networks) {
      if (net.provisional) {
        // eslint-disable-next-line no-await-in-loop
        const nextIp = await getNextProvisioningIP(assignedIps);
        if (nextIp) {
          assignedIps.push(nextIp);
        }
        networks.push({
          ...net,
          address: nextIp || '', // Empty if no IP available
        });
      } else {
  ...
  const nics = [];
  if (sourceConfig.nics) {
    for (const nic of sourceConfig.nics) {
      const nicCopy = { ...nic };
  ...
  return {
    ...
    name: newZoneName, // Base name for task
  };
};
